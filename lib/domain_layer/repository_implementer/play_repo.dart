...
class PlayRepository {
  ...
  Future<Either<Failure, ShowData<ActiveMatch>>> getAllMatches(int end) async {
    try {
      List<Map<String, dynamic>?> groundData =
          await MongoDatabase.instance.getActiveMatches(0, end);
      ShowData<ActiveMatch> data =
          ShowData(groundData.map((e) => ActiveMatch.fromJson(e!)).toList());
      return Right(data);
    } catch (_) {
      return const Left(Failure("Error happened while getting active matches"));
    }
  }
  ...
  Future<Either<Failure, ShowData<ActiveMatch>>> getMoreMatches(
      ShowData<ActiveMatch> old) async {
    try {
      old.getNext();
      List<Map<String, dynamic>?> storeData =
          await MongoDatabase.instance.getActiveMatches(old.start, old.end);
      List<ActiveMatch> matches =
          storeData.map((e) => ActiveMatch.fromJson(e!)).toList();
      old.data.addAll(matches);
      return Right(old);
    } catch (_) {
      return const Left(Failure("Error happened while getting active matches"));
    }
